<?php
	...
	class User extends MY_Controller {
		...
		public function insert_invoices()
		{
			...
			$inputRow[$i]["project"] = $_POST['project'.$inp];
			$inputRow[$i]["amount"] = $_POST['amount'.$inp];
			$inputRow[$i]["number"] = $_POST['number'.$inp];
			$inputRow[$i]["total"] = $_POST['total'.$inp];

			//for each column in current UI row, add a SQL row
				foreach ($inputRow as $index => $currentRow) {
					if ($currentRow["amount"] > 0) {
					$this->Invoices->inv_proid = $currentRow['project'];
					$this->Invoices->inv_amount = $currentRow['amount'];
					$this->Invoices->inv_total = $currentRow['total'];
					$this->Invoices->inv_number = $currentRow['number'];
					
					$this->Invoices->inv_date_modified = date('Y-m-d H:i:s');
					$dtarr = explode("-", $currentRow['date']);	
					$this->Invoices->inv_date = implode("-", array_reverse($dtarr));

					$old_row = $this->Invoices->isInvoiceExists(); // returns empty for new entry
					// die(va);
					if ((count($old_row) > 0)) {
						$this->Invoices->inv_id = $old_row[0]['inv_id'];
						$this->Invoices->inv_date_created = $old_row[0]['inv_date_created'];
					}
					else
						{
							$this->Invoices->inv_date_created = date('Y-m-d H:i:s');
							unset($this->Invoices->inv_id);
						}
					
						if($this->Invoices->save()) {$success++; unset($this->Invoices->inv_id);}
					}
				}
		...
		public function create_resource()
		{
			...
			//for each column in current UI row, add a SQL row
					...
					$this->Resource->res_password = $_POST['password'];
					...
					if (count($old_row) > 0) {
						...
						if ($this->Resource->res_password == "") {
							...
							$this->Resource->res_password = md5($this->Resource->res_password);
						}
		...
		public function add_invoices($search = '') {
		    ...
		    $this->load->model('Invoices');
		    $data['my_invoices'] = $this->Invoices->get_all_invoices();
		...
		public function reports() {
		    ...
		    $data['years'] = array_reverse(range(2000,date("Y")));
		    if ($this->session->userdata('user_type') == 0) {
		    $this->load->model('Project');
		    $this->load->model('Resource');
		    $this->load->model('Timesheet');
		...
		public function get_resource_figures()
		{
		    $this->load->model('Resource');
		    $this->Resource->res_id = $this->input->post('resid');
		    $start = $this->input->post('start');
		    $end = $this->input->post('end');
			echo json_encode($this->Resource->get_figures($start, $end));
		}
		...
		{
			$this->load->model('Invoices');
			$this->Invoices->inv_id = $this->input->post('inv_id');
